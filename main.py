...
try:
    ...
    if llm is None:
        print("--- main.py ERROR: LLM not initialized in set_env.py"); sys.stdout.flush()
    if vector_store is None:
        print("--- main.py ERROR: vector_store not initialized in set_env.py. RAG will not function."); sys.stdout.flush()
    if embeddings_model is None:
        print("--- main.py ERROR: embeddings_model not initialized in set_env.py."); sys.stdout.flush()
    else:
        model_name_display = getattr(embeddings_model, 'model_name', 'Unknown Model')
        print(f"--- main.py: Successfully imported llm, vector_store (for collection '{qdrant_collection_name}'), and embeddings_model '{model_name_display}'."); sys.stdout.flush()

except ImportError as e:
    logging.error(f"--- main.py ERROR: Could not import from set_env.py: {e}", exc_info=True); sys.stdout.flush()
    llm = None
    vector_store = None
    embeddings_model = None
    qdrant_collection_name = "allocations_docs" # Fallback
except Exception as e:
    logging.error(f"--- main.py An unexpected error during imports from set_env: {e}", exc_info=True); sys.stdout.flush()
    llm = None
    vector_store = None
    embeddings_model = None
    qdrant_collection_name = "allocations_docs" # Fallback
...
def retrieve_documents(query: str) -> dict:
    ...
    """
    print(f"--- main.py retrieve_documents TOOL CALLED with query: '{query}' for collection '{qdrant_collection_name}'"); sys.stdout.flush()
    if vector_store is None:
        print("--- main.py retrieve_documents ERROR: vector_store is not available."); sys.stdout.flush()
        return {"error": "Vector store not available.", "documents_content": "Vector store is offline.", "retrieved_docs_metadata": []}
    try:
        retrieved_docs = vector_store.similarity_search(query, k=3)
        print(f"--- main.py retrieve_documents: Retrieved {len(retrieved_docs)} documents."); sys.stdout.flush()
        ...
        logging.error(f"--- main.py retrieve_documents ERROR during similarity search: {e}", exc_info=True); sys.stdout.flush()
...
def query_router_node(state: MessagesState):
    print("--- main.py Query Router Node ---"); sys.stdout.flush()
    log_messages_summary = [f"{msg.type}: {msg.content[:50] + '...' if len(msg.content) > 50 else msg.content}" for msg in state['messages']]
    print(f"--- main.py Query Router Node: Received messages summary: {log_messages_summary}"); sys.stdout.flush()

    if llm is None:
        print("--- main.py Query Router Node: LLM is not available."); sys.stdout.flush()
    ...
    try:
        ...
        print(f"--- main.py Query Router Node: LLM response/tool_call: {ai_response_or_tool_call.type} - Content: {ai_response_or_tool_call.content[:100] if ai_response_or_tool_call.content else 'N/A'} - ToolCalls: {tool_call_summary}"); sys.stdout.flush()
        ...
        if ai_response_or_tool_call.tool_calls:
            for tc in ai_response_or_tool_call.tool_calls:
                if tc['name'] != 'retrieve_documents':
                    ...
                    print(f"--- main.py Query Router Node WARNING: LLM attempted to call an unauthorized tool: {tc['name']}. Overriding to respond: {error_message}"); sys.stdout.flush()
        ...
        logging.error(f"--- main.py Query Router Node: Error invoking LLM with tools: {e}", exc_info=True); sys.stdout.flush()
...
def generate_answer_node(state: MessagesState):
    print("--- main.py Generate Answer Node ---"); sys.stdout.flush()
    if llm is None:
        print("--- main.py Generate Answer Node: LLM is not available."); sys.stdout.flush()
    ...
    print(f"--- main.py Generate Answer Node: Generating final answer for question: '{last_human_message_content}' with context snippet: '{retrieved_context_str[:200]}...' and history snippet: '{formatted_chat_history_for_llm[:100]}...'"); sys.stdout.flush()
    ...
    try:
        ...
        print(f"--- main.py Generate Answer Node: Final AI Answer: {ai_final_answer.content}"); sys.stdout.flush()
        return {"messages": [ai_final_answer]}
    except Exception as e:
        logging.error(f"--- main.py Generate Answer Node: Error invoking LLM for final answer: {e}", exc_info=True); sys.stdout.flush()
        return {"messages": [AIMessage(content=f"Error generating final answer: {e}")]}

# --- Graph Compilation ---
graph = None
if llm and vector_store and embeddings_model:
    print("--- main.py: Compiling LangGraph ---"); sys.stdout.flush()
    ...
    print("--- main.py: Conversational RAG graph compiled successfully with MemorySaver."); sys.stdout.flush()
else:
    # Log which component is missing
    if not llm: print("--- main.py ERROR: LLM is None, cannot compile graph."); sys.stdout.flush()
    if not vector_store: print("--- main.py ERROR: vector_store is None, cannot compile graph."); sys.stdout.flush()
    if not embeddings_model: print("--- main.py ERROR: embeddings_model is None, cannot compile graph."); sys.stdout.flush()
    graph = None
